...
export class CallsService {
    ...
    return new Promise<ICreateCallResponse>((resolve, reject) =>  {
      client.calls
        .create({
          ...
          from: twilioNumber,
          to: hostNumber,
          statusCallback: process.env.NODE_ENV === 'production' ? '' : 'https://1b98a9bc.ngrok.io/api/v1/calls/status-changed',
   ...
   * @param duration
   */
  changeStatus(callSid: string, status: CallStatus, duration: string | number): Promise<void> {
        ...
        .then(res => {
          ...
          switch (status) {
            ...
            case CallStatus.NO_ANSWER:
              ...
              break;
            default:
              payload = 'Your call status is changed but we cannot get it current state.';
          }
